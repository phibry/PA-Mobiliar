...
Next thing to look at is the optimisation with a single simple moving average. For AR models, the start date is a hyperparameter, as is the model order *p*. For SMA models there is no model order but a filter length *L*. The filter length is limited to:

$$5<L<500$$
Additionally the in-sample range is varied again. For each start date the in-sample performances are calculated for each filter length in the interval $5<L<500$. The AR model was only limited to the performance indicator sharpe ratio. Here the optimisation is completed with the drawdown and MSE indicators. Thus, for each start date, for each filter length *L*, the three different indicators are calculated and an optimal model can be choosen.


```{r, chap3.2.3.1, echo=FALSE, fig.cap="Optim", fig.dim=c(11, 6), out.width = "100%", fig.keep = 'last'}
...
```{r, chap3.2.3.3, echo=FALSE, fig.cap="Optim", fig.dim=c(11, 6), out.width = "100%", fig.keep = 'last'}
...
```{r, chap3.2.3.4, echo=FALSE, fig.cap="Best", fig.dim=c(11, 6), out.width = "100%"}
perfplot_sma_opt(xall=log_ret_ind, insamp="2019-01-01", res_obj=res2, inx=2)
```

\newpage

```{r, chap3.2.3.5, echo=FALSE, fig.cap="Optim", fig.dim=c(11, 6), out.width = "100%", fig.keep = 'last'}
...
```{r, chap3.2.3.6, echo=FALSE, fig.cap="Best", fig.dim=c(11, 6), out.width = "100%", fig.keep = 'last'}
perfplot_sma_opt(xall=log_ret_ind, insamp="2019-01-01", res_obj=res3, inx=3)
```

\newpage

```{r, chap3.2.3.7, echo=FALSE, fig.cap="Optim", fig.dim=c(11, 6), out.width = "100%"}
...
```{r, chap3.2.3.8, echo=FALSE, fig.cap="Best", fig.dim=c(11, 6), out.width = "100%"}
...
load("data/R_Files/optim_ma_cross_obj_1_with_xts.RData")
load("data/R_Files/optim_ma_cross_obj_2_with_xts.RData")
load("data/R_Files/optim_ma_cross_obj_3_with_xts.RData")
load("data/R_Files/optim_ma_cross_obj_4_with_xts.RData")###loadind xts datasets index1 

load("data/R_Files/optim_ma_cross_obj_1.RData")
load("data/R_Files/optim_ma_cross_obj_2.RData")
load("data/R_Files/optim_ma_cross_obj_3.RData")
load("data/R_Files/optim_ma_cross_obj_4.RData")#loading perf mat index1


load("data/R_Files/index_1_tenbest_year.RData")#loading tenbest




load("data/R_Files/index_1_onebest_yearwith15.RData")#loading one best
...
By plotting the sharperatio / max drawdown$_1$ in \ref{fig:chap3.2.4.1.4} and the optimal filterlenghts L1/L2 in \ref{fig:chap3.2.4.1.5}, for every insample timestep as described at the beginning of [3.5.1.](#macrossingse), the  Hyperoptimization is visualized. The results from the trajectories in \ref{fig:chap3.2.4.1.2} and \ref{fig:chap3.2.4.1.3}
...
Whilst the ten best sharperatios / drawdowns in \ref{fig:chap3.2.4.1.6} are always very close together, the filterplot \ref{fig:chap3.2.4.1.6}  shows a completely different and interesting image. We can observe differences, specially in the long filters in 2009 2010 2011 the result nearly diverge 150 units. Also in the timespan 2011 to 2018 the filters fluctuate strongly. What further catches the eye are the long and the short filter sharpes from 2003 to 2011. They are all concentrated at their upper limits of 50 and 250, which indicates that they actually wanted to be longer than the restriction. The same behaviour is observed for the short drawdown from 2011 to 2018 where the observations are concentrated around length 1. The plot would shurely look different if the restrictions of the filterlengths would be higher. Also the steps of the insample timespan are always one year. Changeing the restriction would have a massive impact on the optimization time and would most likely deliver different results. But we take the risk in good knowledge of maybe not finding the right filterlenght and use this optimization for trading the series in the next section.


\newpage
As noticed the Visualizations  are only done for index one, the are shown in the tables \ref{tab:macrosstable index 1},\ref{tab:macrosstable index 2},\ref{tab:macrosstable index 3},\ref{tab:macrosstable index 4}. For 
...
Based on the optimal filterlenghts calculatet in the last section the SMAs are now applied to the "out of sample" span from 2019-01-01 to 2020-04-01. Keeping in mind that the Moving Averages need the same 
initialization time as the length of filter itself, the initialization phase of the filter is taken out of sample.
