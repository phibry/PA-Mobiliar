...
load("data/R_Files/index_1_tenbesthead_year.RData")
``` 
...
##################sharpe maxdrow
plot(optim_ma_cross_obj_1_with_xts[,1],ylim= c(-2,4),main= "Max Sharpe/Drawdown Hyperoptimization Index 1",col="black",type="b")
lines(optim_ma_cross_obj_1_with_xts[,4]*100,lwd=2,col="green",type="b")
...
################## lengths
plot(optim_ma_cross_obj_1_with_xts[,2],type="b",col= "red",main= "Filterlengths Hyperoptimization Index 1 ",ylim=c(-10,260))

lines(optim_ma_cross_obj_1_with_xts[,3], col=c("coral"),lwd=2,type="b")
lines(optim_ma_cross_obj_1_with_xts[,5], col=c("blue"),lwd=2,type="b")
lines(optim_ma_cross_obj_1_with_xts[,6], col=c("cornflowerblue"),lwd=2,type="b")
...
For getting a better sense the Hyperoptimazation is again calculatet, this time showing the 10 best results of every insample timespan.
...
\newpage
As noticed the Visualizations are only done for index one, We've done the same optimization for all indexes the result are shown in the table as follows

```{r macrosstable, echo=FALSE}
load("data/R_Files/optim_ma_cross_1.RData")
