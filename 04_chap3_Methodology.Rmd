...
As a third approach we apply 2 SMAs for every time series, trade them with the common Ma crossings rules introduced in [2.3.3.](#macross-section).

##### 3.2.4.1 Optimization

&nbsp;
For the Optimization the same procedure as in the previous sections gets executed.
The optimization has two levels. for different in sample timespans we optimiize the filter lengths on sharpe and maxdrawdown


Hyperparameter:
-$date$ = startdate for the insample timespan $2003-01-01 < date \le 2018-01-01, by 250 steps$
Parameters:
...
###loadind xts datasets
load("data/R_Files/optim_ma_cross_obj_1_xts.RData")
load("data/R_Files/optim_ma_cross_obj_2_xts.RData")
load("data/R_Files/optim_ma_cross_obj_3_xts.RData")
load("data/R_Files/optim_ma_cross_obj_4_xts.RData")
``` 
\newpage

```{r, chap3.2.4.1, echo=FALSE, fig.cap="Hyperoptimization Index 1", include=T,fig.keep = 'last',fig.dim=c(16, 7.1),out.width = "100%"}
##################sharpe maxdrow
...
```
```{r, chap3.2.4.1.1, echo=FALSE, fig.cap="Hyperoptimization Index 1", include=T,fig.keep = 'last',,fig.dim=c(16, 7.1),out.width = "100%"}
################## lengths
...
```
\newpage
```{r, chap3.2.4.1.2, echo=FALSE, fig.cap="Hyperoptimization Index 1", include=T,fig.keep = 'last',,fig.dim=c(16, 7.1),out.width = "100%"}
par(mfrow=c(1,2))
