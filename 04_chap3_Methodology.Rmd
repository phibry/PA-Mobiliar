...
By computing the acf of the squared log_returns we see that the volatility cluster have very long dependency structures f more than a month. 
...
in Section 2 we have learned different indicators and models for timeseries-analysis. Some of these models and indicators are now used to trade the indexes we've introduced in the previous section.
...
As mentioned earlier the goal of this work is trying to outperform the buy and hold strategy. Because the series all have a strong upward trend this task is very tricky. Buy and hold has very low trading costs because the underlyings are just bought once. According to swissquote [@swissquote] costs for asset trades over 50k, are 190 USD per trade $_1$, so these costs should also be taken in consideration for chosing the strategy. In this work we've excluded these costs and only cocncentrate on the trades themselves.
