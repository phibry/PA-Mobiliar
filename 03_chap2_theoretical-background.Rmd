...
The ARCH(*m*)-process can be generalized by adding the lagged conditional variances to the equation \ref{eq:arch}. One gets the GARCH(*n*,*m*)-process.
...
If one wants to improve the the standardized residuals $u_{t}$, an ARIMA part would have to be added to the existing model (see [2.2.3](#stationarity)). This can again be optimized with different model orders. If you want to improve the squared standardized residuals $u^{2}_{t}$, then you should modify the GARCH model order.
...
One can see the divergence to the unconditional standard deviation $\sigma=$ asd `r round(sig, 6)`

#### 2.2.3. ARIMA-GARCH {#arima-garch-section}
...
moving average filters are basically used to identify trends and smooth out price fluctuations. As a commonly used tool moving average filters are very simple in its usage, historical data was summarized and divided bi the length of the filter. 
Many differnt indicators are built on the Moving average principle, mostly theyre used in combinations of different lengths to create new indicators. in the following section we introduce some of the popular indicators based on the MA principle.
...
for a timespan $k$ the last price $p_k$, $k$ timesteps ago is subtractet from the last price. This is equivalent to applying and EqMA to the price differences. 
...
the calculated ${\sigma_t}$ could be problematic because its derived from the original series and increases with the level, its non stationary. Therefore an other method to calculate the standart deviation could be used. As done in  section [2.2.2.](#GARCH) the standart deviation could be provided by a GARCH which means it would handle the volatility.


- ${N}$ = usually the filterlength and the length considered for ${\sigma}$ the same
...
```{r,chap2.9, echo=FALSE, warning=FALSE, message=FALSE, fig.keep = 'last'}
chartSeries(Cl(GOOGL),subset="2007-05::2009-01",theme=chartTheme("white"),name= "MACD EMA GOOGL Closing")
...
in Section 2 we've learned different indicators and models for timeseries. these Models and indicators are now used to trade the underlying asset. As an example we're taking the ma crossings, as mentioned [2.3.3.](#GARCH) the points where the two MAs cross are now used to create a signal. when the longer MA comes from below to the crosspoint we are going to sell the asset and if it approaches the point from above were going to sell. We apply a 1 to a vector at each crosspoint where we intend to buy and apply a -1 at the points we want to sell.