...
The MACD is also a commonly used filter. The basic principle is to subtract a longer EMA with length $L$ as in section   [2.3.3](#macross-section) from a shorter EMA from length $S$  then smooth the result with another EMA with length $R$. As a result we can use the crossing of the 2 generated curves for trading. As an alternative we could use SMA's instead of EMA's
