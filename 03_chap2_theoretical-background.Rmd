...
SMI <- SMI[,6]
par(mfrow=c(1,1))
plot(SMI, main="Adjusted Prices ~ GOOGL")
```
\newpage
...
SMI.lr <- na.exclude(diff(log(SMI)))
plot(SMI.lr, main="Log-Returns of Adj. Prices ~ GOOGL")
```
...
In the following figure \ref{fig:chap2.3} are acf's of the non-stationary adjusted GOOGL stock and their log-returns visualized. Both graphics show the typical pattern of a non-stationary time series. The left plot shows the dependence structure of the time series. This means that it takes a long time until the series changes. Often a large value is followed by another large value, which indicates a strong trend. This property of the series can be seen in figure \ref{fig:chap2.1}. In the right plot the acf of the log-returns are visualized. Remember figure \ref{fig:chap2.2}, there is no trend visible and therefore no long dependency structures are visible, only slight dependencies can be seen.

Later in section [2.2.2.](#arch-garch-section) the characteristics of the autocorrelation function can be used for the verification of ARCH-processes.

```{r chap2.3, echo=FALSE, fig.cap="Acf and Pacf of the LogReturns of GOOGL."}
par(mfrow=c(1,2))
acf(SMI, main="Adjusted Prices ~ GOOGL")
acf(SMI.lr, main="LogReturns ~ GOOGL")
```
...
An ARMA(*p*,*q*) process is defined as follows.
...
The mean of an ARMA-process can be computed as: 

$$\mu=\frac{c}{1-a_{1}-...-a_{p}}$$

Using these processes, forecasts can be generated from stationary time series.
...
Another process is the combination of ARMA and GARCH processes.
...
Is called the mean-equation \ref{eq:arma-garch1}

Is called the variance-equation \ref{eq:arma-garch2}
