...
If you want to adopt a GARCH model and you discover non-vanishing autocorrelations in the standardized residuals $\hat{u}_{t}$, the model can be extended with an ARMA part.
...
Moving average filters are basically used to identify trends and smooth out price fluctuations. As a commonly used tool moving average filters are very simple in their usage, historical data was summarized and divided by the length of the filter. 
Many different indicators are built on the Moving Average principle, mostly they are used in combinations of different lengths to create new indicators. in the following section, we introduce some of the popular indicators based on the MA principle.
