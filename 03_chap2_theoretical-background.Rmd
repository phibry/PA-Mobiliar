...
Moving average filters are basically used to identify trends and smooth out price fluctuations. As a commonly used tool, moving average filters are very simple in their usage, historical data from a timeframe L gets summarized and divided by the length of the filter (L). 
...
A skillfull trader choses an optimal $\alpha$ to increase the performance of the measurement. Weights could also be given individually by adding a weight vector to the filter.
...
An example of MA average crossings with 2 SMAs of different length is visualized in # \ref{fig:chap2.3.3.}. As mentioned earlier, finding the right length of the filter depends on each timeseries and their behaviour. 
...
n1=50
n2=250
chartSeries(GOOGL,type="line",subset="2019-3::2020-10",theme=chartTheme("white"),name= "MA Crossings Google Nasdag")
